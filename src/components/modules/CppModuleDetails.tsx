...
const steps = [
  ...
  { n: 2, title: "Данные подтягиваются автоматически и формируется QR", desc: "Цифровой профиль перевозки со всеми документами формируется из государственных источников — заполнять ничего не нужно. Водитель получает один QR-код.", icon: Smartphone },
      ...
      <div ref={(el) => { refs.current["Было / Стало"] = el; }}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="border border-success/30 rounded-xl p-4 bg-success/5">
            ...
            <ul className="mt-3 space-y-1.5">
              {["Один QR-код", "Документ из государственных баз уже проверен", "Подтверждение по источнику", "Меньше спорных ситуаций"].map((t) => (
                <li key={t} className="flex items-start gap-2">
